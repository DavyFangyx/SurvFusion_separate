"""
Collect Multi_model_test2 results and export summary CSVs.

Supported directory structure:
    results/Multi_model_test2/{condition}/{model}/test_result.csv

Where condition looks like:
    {study}__{modality_pair}__L4__gene_norm__uni_v2

This script outputs into results_display/Multi_model_test2/:
...
2. Dataset-pair summary table:
   Multi_model_test2_summary.csv
   rows = (model_type, model)
   cols = {study}__{modality_pair}
   values = test_cindex formatted as "mean ± std"

3. Flat combined table:
   Multi_model_test2_combined.csv
   rows = one row per (condition, model)
   cols = study / modality_pair / condition / model / model_type / fold_num / metrics
"""
...
MODEL_TYPE_MAP: dict[str, str] = {
    "survtri_snn_concat": "SNN",
    "survtri_mlp_concat": "MLP",
    "survtri_mlp_mhsa": "MLP",
}
...
def _sort_key_model(model: str) -> tuple[int, int, str, str]:
    base_model, combo = split_model_name(model)
    return (
        MODEL_ORDER.index(base_model) if base_model in MODEL_ORDER else 99,
        PAIR_ORDER.index(combo) if combo in PAIR_ORDER else 99,
        base_model,
        combo,
    )
...
def _sort_key_condition(condition: str) -> tuple[str, int, str]:
    ...
    study = str(parsed["study"])
    heads = parsed["heads"]
    pair = str(parsed["modality_pair"])
    if heads is not None:
        return (study, int(heads), condition)
    return (
        study,
        PAIR_ORDER.index(pair) if pair in PAIR_ORDER else 99,
        condition,
    )
...
def parse_condition_name(condition: str) -> dict[str, object]:
    ...
    modality_pair = parts[1] if len(parts) > 1 else ""
    config_tail = "__".join(parts[2:]) if len(parts) > 2 else ""
    ...
    parsed["study_pair"] = (
        f"{parsed['study']}__{parsed['modality_pair']}"
        if parsed["modality_pair"]
        else str(parsed["study"])
    )
    parsed["study_heads"] = (
        f"{parsed['study']}__h{int(parsed['heads']):02d}"
        if parsed["heads"] is not None
        else str(parsed["study_pair"])
    )
    return parsed
...
def split_model_name(model_name: str) -> tuple[str, str]:
    if "__" not in model_name:
        return model_name, ""
    base_model, combo = model_name.rsplit("__", 1)
    return base_model, combo


def build_condition_label(parsed: dict[str, object]) -> str:
    if parsed["heads"] is not None:
        return str(parsed["study_heads"])
    return str(parsed["study_pair"])
...
def build_condition_table(condition_dir: Path, metrics: list[str]) -> pd.DataFrame | None:
    ...
    for model_dir in sorted(condition_dir.iterdir(), key=lambda p: _sort_key_model(p.name)):
        ...
        if row is None:
            continue
        base_model, combo = split_model_name(model_dir.name)
...
def build_summary(results_dir: Path, conditions: list[str]) -> pd.DataFrame:
    ...
    for condition in conditions:
        ...
        parsed = parse_condition_name(condition)
        summary_col = build_condition_label(parsed)
        ...
        for model_dir in sorted(condition_dir.iterdir(), key=lambda p: _sort_key_model(p.name)):
            ...
            model, _ = split_model_name(model_dir.name)
    ...
    for condition in conditions:
        ...
        for model_dir in sorted(condition_dir.iterdir(), key=lambda p: _sort_key_model(p.name)):
            ...
            row: dict[str, object] = {
                ...
                "condition": condition,
                "study": parsed["study"],
                "modality_pair": parsed["modality_pair"],
                "config_tail": parsed["config_tail"],
                "heads": parsed["heads"],
                "study_pair": parsed["study_pair"],
                "model_type": MODEL_TYPE_MAP.get(base_model, "其他"),
                "model": base_model,
                "model_combo": combo,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Collect Multi_model_test2 result summaries")
    parser.add_argument(
        "--results_dir",
        type=Path,
        default=project_root / "results" / "Multi_model_test2",
        help="Path to Multi_model_test2 directory",
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=project_root / "results_display" / "Multi_model_test2",
