...
# Required Files
clinical_1 = pd.read_csv('./MEDHIST_09May2024.csv')
clinical_2 = pd.read_csv('./NEUROEXM_09May2024.csv')
clinical_3 = pd.read_csv('./PTDEMOG_09May2024.csv')
clinical_4 = pd.read_csv('./RECCMEDS_09May2024.csv')
clinical_5 = pd.read_csv('./VITALS_09May2024.csv')
...
# List of clinical DataFrames
clinicals = [clinical_1, clinical_2, clinical_3, clinical_4, clinical_5]

# Convert column names of each DataFrame to uppercase
for i in range(len(clinicals)):
    clinicals[i].columns = clinicals[i].columns.str.upper()
...
cols_1 = set(clinicals[0].columns)
cols_2 = set(clinicals[1].columns)
cols_3 = set(clinicals[2].columns)
cols_4 = set(clinicals[3].columns)
cols_5 = set(clinicals[4].columns)

common_cols = cols_1.intersection(cols_2, cols_3, cols_4, cols_5)
...
final.to_csv('clinical_merged.csv', index=True)
...
final_2.to_csv('clinical_merged_mean.csv', index=True)
